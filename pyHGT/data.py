...
class Graph():
    ...
    def add_edge(self, source_node, target_node, relation_type=None, directed=True):
        edge = [self.add_node(source_node), self.add_node(target_node)]
        '''
            Add bi-directional edges with different relation type
        '''
        self.edge_list[target_node['type']][source_node['type']][relation_type][edge[1]].append(edge[0])
        if directed:
            self.edge_list[source_node['type']][target_node['type']]['rev_' + relation_type][edge[0]].append(edge[1])
        else:
            self.edge_list[source_node['type']][target_node['type']][relation_type][edge[0]].append(edge[1])
        ...
        for source_type in te:
            ...
            for relation_type in tes:
                ...
                if len(adl) < sampled_number:
                    sampled_ids = list(adl)
                else:
                    sampled_ids = np.random.choice(list(adl), sampled_number, replace=False)
